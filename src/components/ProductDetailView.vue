...
const currentIndex = computed(() =>
    allProducts.value.findIndex(p => p.id === Number(props.id))
)

const previousProduct = computed(() =>
    currentIndex.value > 0 ? allProducts.value[currentIndex.value - 1] : null
)

const nextProduct = computed(() =>
    currentIndex.value !== -1 && currentIndex.value < allProducts.value.length - 1
        ? allProducts.value[currentIndex.value + 1]
        : null
)
